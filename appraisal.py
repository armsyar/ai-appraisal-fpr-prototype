...
# --- Consent gate (new in this file; not present in app.py) ---
CONSENT_STATEMENTS = [
    "I confirm that I have read the study information. I have had the "
    "opportunity to consider the information and ask questions. Any "
    "questions have been answered satisfactorily.",
    "I understand that my participation is voluntary and I am free to "
    "withdraw at any time without giving a reason.",
    "I understand that my data will be stored securely on UH-approved "
    "systems, anonymized, and securely destroyed at the end of the "
    "research and assessment process.",
    "I am 18 or over and have experience of conducting or contributing "
    "to annual performance appraisals in a managerial role.",
]


def _log_consent(session_id: str) -> None:
    """
    Appends one row to a local consent_log.csv: session id + UTC timestamp.
    Deliberately kept separate from storage.py's save_response/FIELDNAMES,
    which are fixed to the questionnaire-response shape and were not
    changed for this. This is a self-contained addition local to this file.
    """
    import csv
    import os

    fieldnames = ["participant_session_id", "consented_at_utc"]
    file_exists = os.path.exists("consent_log.csv")
...
def render_consent_gate() -> bool:
    """Returns True once the participant has agreed to every statement."""
    if st.session_state.get("consent_given"):
        return True

    st.title("📋 Participant Information and Consent")
    st.markdown(
        "You are being invited to take part in an online study as part of "
        "a Master's course being undertaken by Amsyar Ramlee, an MSc "
        "Computer Science with Artificial Intelligence student at the "
        "School of Physics, Engineering and Computer Science, University "
        "of Hertfordshire, UK. Please read the information below before "
        "deciding whether to take part."
    )
    st.markdown(
        "**The study.** This study evaluates an AI-assisted prototype that "
        "helps managers draft annual employee performance appraisal "
        "summaries, and investigates whether AI-generated summaries are "
        "accurate and evidence-grounded, and how transparency affects "
        "managers' perceptions of fairness, trust, and usefulness.\n\n"
        "**What's involved.** You will review synthetic (fully fictional) "
        "employee performance cases and write appraisal summaries — once "
        "with AI assistance, once without — for at least one case (up to "
        "four), then answer a short questionnaire after each step. Total "
        "time: roughly 30–45 minutes for one case.\n\n"
        "**Voluntary participation.** Taking part is entirely up to you. "
        "You may withdraw at any time without giving a reason.\n\n"
        "**Confidentiality.** No real employee data is used at any point. "
        "Your responses are anonymous — this tool never asks for your "
        "name, login, or any other identifying information.\n\n"
        "**Ethics approval.** This study has been approved by the "
        "University of Hertfordshire Health, Science, Engineering and "
        "Technology Ethics Committee with Delegated Authority, protocol "
        "number **2570 ST HSET 2026**.\n\n"
        "**Questions.** Researcher: Amsyar Ramlee (ar22aej@herts.ac.uk). "
        "Supervisor: Dr. Bahja (m.bahja@herts.ac.uk)."
    )
    st.divider()
    st.subheader("Please confirm each statement to proceed")

    checks = [
        st.checkbox(stmt, key=f"consent_check_{i}")
        for i, stmt in enumerate(CONSENT_STATEMENTS)
    ]

    if st.button("✅ I agree — begin the study", type="primary"):
        if all(checks):
            st.session_state["consent_given"] = True
            _log_consent(st.session_state["participant_session_id"])
            st.rerun()
        else:
            st.warning(
                "Please confirm all four statements above before "
                "continuing. If you do not wish to proceed, you may "
